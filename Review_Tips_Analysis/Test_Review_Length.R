##whether we should include document length as a variable 
...
svd_bin_cv_multinomial_lasso<- cv.glmnet(SVD_bin_reconstruct, train_reviews$stars, family = "multinomial", nfolds = 5, alpha=1,
                                         ...
                                         maxit = 1e4,
                                         parallel = TRUE)
print(difftime(Sys.time(), t1, units = 'min')) #21.236 minutes
...
sum(svd_bin_multinomial_lasso_predict == test_reviews$stars)/length(test_reviews$stars) #0.54525

svd_bin_multinomial_lasso_cf = confusionMatrix(test_reviews$stars,svd_bin_multinomial_lasso_predict)
svd_bin_multinomial_lasso_cf #Precision:  0.7142   0.1818   0.2040    0.432    0.786
...
mean((mean_pred_svd_bin_multinomial_df$stars-mean_pred_svd_bin_multinomial_df$mean_pred)^2) #0.21799
