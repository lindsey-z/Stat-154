#does including "useful" as a variable improve prediction
...
doc_length_train = train_reviews %>% 
  dplyr::select(X,text) %>%
...
SVD_bin_train = cbind(SVD_bin_train,sign(train_reviews$useful))
colnames(SVD_bin_train)[ncol(SVD_bin_train)] = "useful_variable"
SVD_bin_train = cbind(SVD_bin_train,sqrt(doc_length_train$n))
colnames(SVD_bin_train)[ncol(SVD_bin_train)] = "doc_length"
SVD_bin_train[,ncol(SVD_bin_train)] = scale(SVD_bin_train[,ncol(SVD_bin_train)])
...
SVD_bin_test = cbind(SVD_bin_test,sign(test_reviews$useful))
colnames(SVD_bin_test)[ncol(SVD_bin_test)] = "useful_variable"
SVD_bin_test = cbind(SVD_bin_test,sqrt(doc_length_test$n))
colnames(SVD_bin_test)[ncol(SVD_bin_test)] = "doc_length"
SVD_bin_test[,ncol(SVD_bin_test)] = scale(SVD_bin_test[,ncol(SVD_bin_test)])
...
svd_bin_cv_multinomial_lasso<- cv.glmnet(SVD_bin_reconstruct, train_reviews$stars, family = "multinomial", nfolds = 5, alpha=1,
                                         ...
                                         maxit = 1e4,
                                         parallel = TRUE)
print(difftime(Sys.time(), t1, units = 'min')) #22.186 minutes
...
sum(svd_bin_multinomial_lasso_predict == test_reviews$stars)/length(test_reviews$stars) #0.54654

svd_bin_multinomial_lasso_cf = confusionMatrix(test_reviews$stars,svd_bin_multinomial_lasso_predict)
svd_bin_multinomial_lasso_cf #Precision:   0.7132   0.1832   0.2098    0.434    0.786
...
mean((mean_pred_svd_bin_multinomial_df$stars-mean_pred_svd_bin_multinomial_df$mean_pred)^2) #0.22701
